...
def get_serial_port():
    ...
    command = subprocess.Popen(
        "ls /dev/ttyUSB*", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
...
def get_emf():
    ...
    rf_density, rf_density_mhz_frequency = get_rf_density_and_mhz_frequency(
        rf_density_words
    )
    ...
    return (
        float(rf_watts),
        float(rf_watts_mhz_frequency),
        float(rf_density),
        float(rf_density_mhz_frequency),
        float(rf_total_density),
        float(ef_volts_per_meter),
